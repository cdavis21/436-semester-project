...
public class PurchaseAdapter extends BaseAdapter {
	...
	static class ViewHolderPurchase {
		...
		TextView tvChina, tvBrazil, tvIndonesia, tvIndia, tvPakistan;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			if (getItemViewType(position) == 0) {
				...
				vhp.tvChina = (TextView) convertView.findViewById(R.id.ChinaView);
				vhp.tvBrazil = (TextView) convertView.findViewById(R.id.BrazilView);				
				vhp.tvIndia = (TextView) convertView.findViewById(R.id.IndiaView);			
				vhp.tvIndonesia = (TextView) convertView.findViewById(R.id.IndonesiaView);
				vhp.tvPakistan = (TextView) convertView.findViewById(R.id.PakistanView);			
		...
		if (currentPurchase != null) {
			...
			if(getItemViewType(position) == 2){
				...
				if(!MainActivity.enabledBrazil) {
					vhp.tvBrazil.setText("Disabled");
				} else {
					vhp.tvBrazil.setText(nf.format((currentPurchase.calculateBrazil(currentPurchase.getCost()))));
				}
				
				//India
				if(!MainActivity.enabledChina) {
					vhp.tvChina.setText("Disabled");
				} else {
					vhp.tvChina.setText(nf.format((currentPurchase.calculateChina(currentPurchase.getCost()))));
				}
				
				if(!MainActivity.enabledIndia) {
					vhp.tvIndia.setText("Disabled");
				} else {
					vhp.tvIndia.setText(nf.format((currentPurchase.calculateIndia(currentPurchase.getCost()))));
				}
				
				if(!MainActivity.enabledIndonesia) {
					vhp.tvIndonesia.setText("Disabled");
				} else {
					vhp.tvIndonesia.setText(nf.format((currentPurchase.calculateIndonesia(currentPurchase.getCost()))));
				}
				
				if(!MainActivity.enabledPakistan) {
					vhp.tvPakistan.setText("Disabled");
				} else {
					vhp.tvPakistan.setText(nf.format((currentPurchase.calculatePakistan(currentPurchase.getCost()))));
				}
